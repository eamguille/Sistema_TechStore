...
BEGIN
    ...
    -- Obtenemos el código más alto utilizado en el campo id_usuario
    SELECT @ultimo_codigo = ISNULL(MAX(CAST(SUBSTRING(id_usuario, 2, 5) AS INT)), 0) + 1
    FROM Usuarios;

    -- Generamos el nuevo código concatenando el prefijo 'U' para usuarios
    SET @nuevo_codigo = CONCAT('U', RIGHT('00000' + CAST(@ultimo_codigo AS VARCHAR(5)), 5));

    -- Insertamos el nuevo registro con el código generado
    INSERT INTO Usuarios (id_usuario, nombres, apellidos, nombre_usuario, clave, email, telefono, direccion, rol)
    SELECT @nuevo_codigo, nombres, apellidos, nombre_usuario, clave, email, telefono, direccion, rol
    FROM inserted;
END;

INSERT INTO Usuarios(nombres, apellidos, nombre_usuario, clave, email, telefono, direccion, rol)
VALUES ('Guillermo', 'Castillo', 'eamguille', '8d969eef6ecad3c29a3a629280e686cf0c3f5d5a86aff3ca12020c923adc6c92', 'eamguille@gmail.com', '7633-5320', 'Planes de renderos', 'root')


-- Aqui creamos el PROCEDURE para el login del usuario
